  ...
  <div class='wrapper'>
    ...
    <el-dialog title="评论回复" :visible.sync="dialogBox.boxShow" width='720px' :lock-scroll="true">
      <div class="comment-box">
        <empty-show 
         :hide='dialogBox.detailItem.replyCommentList && dialogBox.detailItem.replyCommentList.length === 0' 
         showTxt='暂无评论' 
         imgWidth='100px' 
         imgHeight='90px'>
        </empty-show>
        <!-- 回复详情列表 -->
        <div class="box-list">
            <div class="list-item" v-for='(item) in dialogBox.detailItem.replyCommentList' :key='item._id'>
              <!-- 路径不明确 -->
                ...
                <div class="item-content">
                  <div class="content-operate">
                    ...
                    <div class="operate-data">
                      <el-button v-if='authList.includes("1")' :type="item.status === '1'?'info':'success'" size="mini" :icon="item.status === '1'?'el-icon-circle-close':'el-icon-circle-check'" @click='replyCommentStatus(item)'>{{item.status === '1'?'禁用':'启用'}}</el-button>
                      <el-button v-if='authList.includes("1")' type="primary" size="mini" icon="el-icon-edit" @click='replyCommentEdit(item)'>编辑</el-button>
                      <el-button v-if='authList.includes("1")' type="warning" size="mini" icon="el-icon-edit-outline" @click='replyComment(item)'>回复</el-button>
                      <el-button v-if='authList.includes("2")' type="danger" size="mini" icon="el-icon-delete" @click='replyCommentDel(item._id)'>删除</el-button>
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="replyCommentAdd" v-if='authList.includes("3")'>新增</el-button>
...
export default {
              ...
              return h('div', {
                ...
                [
                  h('el-button', {
                    ...
                    style: {
                      display: this.authList.includes('1')?'block':'none'
                    },
                    on: {
                      click: () => {
  ...
  created() {
    this.searchForm.operate[0].hide = !this.authList.includes('3')
  },
  mounted() {
